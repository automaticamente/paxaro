...
class Feeder {
    constructor(startTime, outputFolder, number) {
        this.startTime = moment().set('hour', startTime.getHours()).set('minute', 0).utc();
    ...
    _getTimestamp(delta, unit) {
        unit = unit || 'minutes';
        return this.startTime.subtract(delta, unit).format(this.dateFormat);
    }

    _getImage(timestamp, number) {
        return new Promise((resolve, reject) => {
            editor(request(this.baseUrl + timestamp), this.startTime.local().format('DD/MM/YY HH:mm'))
                .then(stream => {
